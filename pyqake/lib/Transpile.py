...
class PySCFTranspiler:
    def __init__(self, mf, active_space=None, active_e=None):
        ...
        if self.active:
            core_orb_alpha      = np.array(range(self.N_alpha - active_e[0]))
            core_orb_beta       = np.array(range(self.N_beta  - active_e[1]))
            active_orb_alpha    = np.array(range(self.N_alpha - active_e[0], self.N_alpha - active_e[0] + active_space))
            active_orb_beta     = np.array(range(self.N_beta  - active_e[1], self.N_beta  - active_e[1] + active_space))
