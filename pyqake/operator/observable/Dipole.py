...
def run(mf_qc, Estimator, L=None, R=None, mapping='jordan_wigner', ignore_core=False):
    ...
    D_X                         = SparsePauliOp.simplify(L.conjugate().transpose() @ X @ R)
    D_Y                         = SparsePauliOp.simplify(L.conjugate().transpose() @ Y @ R)
    D_Z                         = SparsePauliOp.simplify(L.conjugate().transpose() @ Z @ R)
