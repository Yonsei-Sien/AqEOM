    ...
    for N_RDM_beta in range(Degree + 1):
        for beta_case in itertools.combinations(ind_lst, N_RDM_beta):
            ...
            for beta_ind in beta_case:
                spin_shifter[beta_ind]                  = N_orb
                spin_shifter[2 * Degree - beta_ind - 1] = N_orb
...
def _generate(C, D, Indices, Degree):
    ...
    def mult_ops(A, B):
        return A @ B
    T_before_product    = [C[a] for a in reordered_indices[:Degree]] + [D[b] for b in reordered_indices[Degree:]] 
