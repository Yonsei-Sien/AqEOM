...
def generate(classic_object, mapping='jordan_wigner'):
    ...
    # A frozen, doubly-occupied core orbital always has both its alpha and
    # beta spin-orbital filled, so any S+/S- ladder term acting on it is
    # annihilated by the Pauli principle -- unlike the Hamiltonian's
    # two-electron integrals, the core contributes no mean-field (J/K)
    # correction to S+S-/S-S+. So under active space, S_mo is restricted
    # to the active orbitals directly; classic_object.apply_active_space
    # is NOT used here, since that Coulomb/exchange folding is only valid
    # for genuine two-electron integral tensors.
    if classic_object.active:
        S_mo    = S_mo[np.ix_(classic_object.active_orb[0], classic_object.active_orb[1])]
