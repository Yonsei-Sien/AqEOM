...
N_state             = 3
CAS_matches         = {0.5: [3, 4], 0.6: [2, 4], 0.7: [2, 4], 0.8: [2, 5], 0.9: [2, 5], 1.0: [2, 5], 1.1: [2, 5], 1.2: [2, 5], 1.3: [2, 5], 1.4: [2, 5], 1.5: [2, 5], 1.6: [2, 5]}
...
energy_qEOM_diag[1:]    = np.sort(mf_qEOM.e[mf_qEOM.e > 0])[[1, 4]] + mf_qc.energy
print("qEOM Done")

# qEOM
for ex in range(N_state - 1):
    O                               = mf_qEOM.gen_state_transfer_op(excitation_level = CAS_match[ex])                                                   
    O_dgr                           = mf_qEOM._gen_state_transfer_op(excitation_level = -CAS_match[ex])      
    Effective_H                     = O_dgr @ mf_qc.H @ O
    Denominator                     = O_dgr @ O
...
np.save(f"/Data/H2O/Energy/{dist}_LR_CASCI_MATCH_qEOM_Diag", energy_qEOM_diag)
np.save(f"/Data/H2O/Energy/{dist}_LR_CASCI_MATCH_qEOM"     , energy_qEOM)
np.save(f"/Data/H2O/Energy/{dist}_LR_CASCI_MATCH_fqEOM"    , energy_fqEOM)