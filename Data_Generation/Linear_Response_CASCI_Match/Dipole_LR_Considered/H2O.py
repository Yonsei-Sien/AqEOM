...
for N_R in range(N_state):
    R_O = mf_qEOM._gen_state_transfer_op(excitation_level        = CAS_match[N_R])
    for N_L in range(N_R + 1):
        L_O = mf_qEOM._gen_state_transfer_op(excitation_level    = -CAS_match[N_L]).conjugate().transpose()
...
np.save(f"/Data/H2O/Dipole/{dist}_LR_CASCI_MATCH_qEOM" , dipole_qEOM)
np.save(f"/Data/H2O/Dipole/{dist}_LR_CASCI_MATCH_fqEOM", dipole_fqEOM)