...
energy_qEOM_diag[1:]    = np.sort(mf_qEOM.e[mf_qEOM.e > 0])[:N_state-1] + mf_qc.energy
print("qEOM Done")
...
np.save(f"/Data/LiH/Energy/{dist}_qEOM_Diag", energy_qEOM_diag)
np.save(f"/Data/LiH/Energy/{dist}_qEOM"     , energy_qEOM)
np.save(f"/Data/LiH/Energy/{dist}_fqEOM"    , energy_fqEOM)