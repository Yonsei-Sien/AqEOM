...
for ex in range(N_state):
    RDM_qEOM            = np.load(f"/Data/LiH/Density/qEOM_{ex}_{dist}.npy")
    RDM_fqEOM           = np.load(f"/Data/LiH/Density/fqEOM_{ex}_{dist}.npy")
...
np.save(f"/Data/LiH/Energy/{dist}_qEOM_{xc}" , energy_qEOM)
np.save(f"/Data/LiH/Energy/{dist}_fqEOM_{xc}", energy_fqEOM)