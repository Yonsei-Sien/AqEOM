...
# everything is correct !!! (symmetry-required degeneracies) -- PySCF and qEOM
# independently pick arbitrary bases within a degenerate subspace, so check
# sum_i Ovl(i,j)^2 over the degenerate block instead of trusting a single diagonal
# entry.

# !!! Active space here is (6 orbitals, 4+4 electrons) -> 12 qubits, 225-dim CASCI
# space (C(6,4)^2). N_states is kept small below -- each state costs one dense
# 4096x4096 Operator build; raise it gradually and re-time rather than jumping high.

# !!! Please handle active space manually !!!

dist            = np.round(float(sys.argv[1]), 1)  # O-H bond length (fixed 104.5 deg angle)
master_dir      = '***'
system          = 'H2O'
active_space    = 6
active_e        = (4, 4)  # O 1s core (2 electrons) frozen

N_states        = 6  # keep small first pass -- 12-qubit Operator builds are the
                      # expensive step here, unlike H2/LiH
