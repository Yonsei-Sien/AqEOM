...
mf              = None
if do_HF:
    mol         = gto.M(atom=f'N 0 0 0; H {dist * (np.sqrt(8) / 3)} 0 {-dist / 3}; H {-dist * (np.sqrt(2) / 3)} {dist * (np.sqrt(6) / 3)} {-dist / 3}; H {-dist * (np.sqrt(2) / 3)} {-dist * (np.sqrt(6) / 3)} {-dist / 3}', 
                        basis='sto3g', spin=0, charge=0)
    mf          = scf.RHF(mol)
...
Transfiled      = PySCFTranspiler(mf, 4, (1, 1)) # !!! Active Space Here !!!
print("HF Done")
